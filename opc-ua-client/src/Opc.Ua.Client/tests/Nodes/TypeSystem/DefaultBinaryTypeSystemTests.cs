    ...
    public async Task LoadAsyncShouldLoadNoTypeDefinitionsFromZeroLengthDictionaryAndNotThrowAsync()
    {
        ...
        _nodeCacheMock
            .Setup(nc => nc.GetReferencesAsync(
                ...
                false,
                ct))
            .ReturnsAsync(new List<INode> { dictionaryNode })
            .Verifiable(Times.Once);
        _nodeCacheMock
            .Setup(nc => nc.GetValuesAsync(
                new List<NodeId> { dictionaryNodeId },
                ct))
            .ReturnsAsync(new List<DataValue> { dictionaryContent })
            .Verifiable(Times.Once);
        _nodeCacheMock
            .Setup(nc => nc.GetReferencesAsync(
                dictionaryNodeId,
                ...
                false, false,
                ct))
            .ReturnsAsync(new List<INode> { nsNode })
            .Verifiable(Times.Once);
        _nodeCacheMock
    ...
    public async Task LoadAsyncShouldLoadTypeDefinitionsFromMachineryAndGmsDictionariesAsync()
    {
        ...
        _nodeCacheMock
            .Setup(nc => nc.GetReferencesAsync(
                ...
                false,
                ct))
            .ReturnsAsync(new List<INode> { machineryDictionary, gmsDictionary })
            .Verifiable(Times.Once);
        _nodeCacheMock
            .Setup(nc => nc.GetValuesAsync(
                new List<NodeId> { machineryDictionaryId, gmsDictionaryId },
                ct))
            .ReturnsAsync(new List<DataValue> { machinary, gms })
            .Verifiable(Times.Once);
        _nodeCacheMock
            .Setup(nc => nc.GetReferencesAsync(
                ...
                false, false,
                ct))
            .ReturnsAsync(new List<INode> { machineryNsNode })
            .Verifiable(Times.Once);
        _nodeCacheMock
            .Setup(nc => nc.GetReferencesAsync(
                gmsDictionaryId,
                ...
                false, false,
                ct))
            .ReturnsAsync(new List<INode> { gmsNsNode })
            .Verifiable(Times.Once);
        _nodeCacheMock
            .Setup(nc => nc.GetValueAsync(
        ...
        _nodeCacheMock
            .Setup(nc => nc.GetValuesAsync(
                ...
                ct))
            .ReturnsAsync(new List<DataValue>
            {
