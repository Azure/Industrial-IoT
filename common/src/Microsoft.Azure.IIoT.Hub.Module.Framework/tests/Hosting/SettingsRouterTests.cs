            ...
            await ModuleHostHarness.RunTestAsync(controller.YieldReturn(),
                async (deviceId, moduleId, services) =>
                {
                    ...
                    await hub.UpdatePropertyAsync(deviceId, moduleId,
                        nameof(TestController1.TestSetting1), test).ConfigureAwait(false);
            ...
            await ModuleHostHarness.RunTestAsync(controller.YieldReturn(),
                async (deviceId, moduleId, services) =>
                {
                    ...
                    await hub.UpdatePropertyAsync(deviceId, moduleId,
                        nameof(TestController1.TestSetting1), test).ConfigureAwait(false);
                    await hub.UpdatePropertyAsync(deviceId, moduleId,
                        nameof(TestController1.TestSetting2), test2).ConfigureAwait(false);
                    var twin = await hub.GetAsync(deviceId, moduleId).ConfigureAwait(false);
            ...
            await ModuleHostHarness.RunTestAsync(controller.YieldReturn(),
                async (deviceId, moduleId, services) =>
                {
                    ...
                    await hub.UpdatePropertyAsync(deviceId, moduleId,
                        nameof(TestController1.TestSetting2), test2).ConfigureAwait(false);
                    await hub.UpdatePropertyAsync(deviceId, moduleId,
                        nameof(TestController1.TestSetting3), test3).ConfigureAwait(false);
                    var twin = await hub.GetAsync(deviceId, moduleId).ConfigureAwait(false);
        ...
        public async Task TestSettingDesiredPropertyAndCheckReported2Async()
        {
            ...
            await ModuleHostHarness.RunTestAsync(controller.YieldReturn(),
                async (deviceId, moduleId, services) =>
                {
                    ...
                    // Act
                    await hub.UpdatePropertyAsync(deviceId, moduleId,
                        nameof(TestController2.TestSetting), test).ConfigureAwait(false);
                    var twin = await hub.GetAsync(deviceId, moduleId).ConfigureAwait(false);
