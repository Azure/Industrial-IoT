    ...
    public sealed class IoTHubFileNotificationHost : IHostProcess, IDisposable {
        ...
        public async Task StartAsync() {
            try {
                ...
                _logger.Debug("Starting file notification host...");
                await _client.OpenAsync();
                var receiver = _client.GetFileNotificationReceiver();
                _task = Task.Run(() => RunAsync(receiver));
                _logger.Information("File notification host started.");
            }
            catch (Exception ex) {
        ...
        public async Task StopAsync() {
            ...
            try {
                ...
                if (_task != null) {
                    _logger.Debug("Stopping file notification host...");
                    await _task;
                    _logger.Information("File notification host stopped.");
                }
            }
            catch (Exception ex) {
                _logger.Warning(ex, "Failed to stop file notification host.");
            }
            finally {
                _task = null;
            }
