...
namespace System.Security.Cryptography.Asn1 {
    ...
    internal class AsnReader {
        ...
        private static int ParseNonNegativeIntAndSlice(ref ReadOnlySpan<byte> data, int bytesToRead) {
            var value = ParseNonNegativeInt(Slice(data, 0, bytesToRead));
        ...
        private static int ParseNonNegativeInt(ReadOnlySpan<byte> data) {
            if (Utf8Parser.TryParse(data, out uint value, out var consumed) && value <= int.MaxValue && consumed == data.Length) {
                return (int)value;
            }
        ...
        private DateTimeOffset ParseUtcTime(ReadOnlySpan<byte> contentOctets, int twoDigitYearMax) {
            ...
            var contents = contentOctets;

            var year = ParseNonNegativeIntAndSlice(ref contents, 2);
            var month = ParseNonNegativeIntAndSlice(ref contents, 2);
            var day = ParseNonNegativeIntAndSlice(ref contents, 2);
            var hour = ParseNonNegativeIntAndSlice(ref contents, 2);
            var minute = ParseNonNegativeIntAndSlice(ref contents, 2);
            var second = 0;
            var offsetHour = 0;
            var offsetMinute = 0;
            var minus = false;
            ...
            var century = twoDigitYearMax / 100;

            if (year > twoDigitYearMax % 100) {
                century--;
            }

            var scaledYear = (century * 100) + year;
        ...
        /// </returns>
        /// <seealso cref="Globalization.Calendar.TwoDigitYearMax"/>
        public DateTimeOffset GetUtcTime(Asn1Tag expectedTag, int twoDigitYearMax = 2049) {
            ...
            var value = ParseUtcTime(contents, twoDigitYearMax);
        ...
        private static DateTimeOffset ParseGeneralizedTime(
            ...
            var strict = ruleSet == AsnEncodingRules.DER || ruleSet == AsnEncodingRules.CER;
            ...
            var year = ParseNonNegativeIntAndSlice(ref contents, 4);
            var month = ParseNonNegativeIntAndSlice(ref contents, 2);
            var day = ParseNonNegativeIntAndSlice(ref contents, 2);
            var hour = ParseNonNegativeIntAndSlice(ref contents, 2);
            int? minute = null;
            ...
            if (state == FracState) {
                ...
                if (!Utf8Parser.TryParse(SliceAtMost(contents, 12), out fraction, out var fracLength) || fracLength == 0) {
                    throw new CryptographicException(SR.Cryptography_Der_Invalid_Encoding);
                }
            ...
            if (state == SuffixState) {
                ...
                if (octet == 'Z') {
                    ...
                    var offsetHour = ParseNonNegativeIntAndSlice(ref contents, 2);
                    var offsetMinute = 0;
                    ...
                    var tmp = new TimeSpan(offsetHour, offsetMinute, 0);
            ...
            var frac = (double)fraction / fractionScale;
            var fractionSpan = TimeSpan.Zero;
        ...
        public DateTimeOffset GetGeneralizedTime(Asn1Tag expectedTag, bool disallowFractions = false) {
            ...
            var value = ParseGeneralizedTime(_ruleSet, contents, disallowFractions);
