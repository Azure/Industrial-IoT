...
namespace Microsoft.Azure.IIoT.Module.Framework.Client.Tests
{
    ...
    public class MqttClientAdapterTests : MqttClientConnectionStringBuilderTestsBase
    {
        ...
        public async Task ConnectTest()
        {
            ...
            await MqttClientAdapter.CreateAsync(mock.Object, _mqttClientConnectionStringBuilder, "device1", "/topic/{device_id}", TimeSpan.Zero, _logger, _metrics).ConfigureAwait(false);
            ...
            mock.Verify(x => x.StartAsync(
                It.Is<ManagedMqttClientOptions>(x =>
                    ...
                    string.Equals(x.ClientOptions.ChannelOptions.As<MqttClientTcpOptions>().Server, "hub1.azure-devices.net", StringComparison.OrdinalIgnoreCase) &&
            ...
            mock.Verify(x => x.SubscribeAsync(
                It.Is<ICollection<MqttTopicFilter>>(x =>
                    x.Count == 3 &&
                    string.Equals(x.First().Topic, "$iothub/twin/res/#", StringComparison.Ordinal) &&
                    string.Equals(x.Skip(1).First().Topic, "$iothub/twin/PATCH/properties/desired/#", StringComparison.Ordinal) &&
                    string.Equals(x.Last().Topic, "$iothub/methods/#", StringComparison.Ordinal))));
        ...
        public async Task SendIoTHubEventTest()
        {
            ...
            mock.Verify(x => x.EnqueueAsync(
                It.Is<MqttApplicationMessage>(x =>
                    string.Equals(x.ContentType, "application/json", StringComparison.Ordinal) &&
                    string.Equals(x.Topic, "devices/device1/messages/events/iothub-content-type=application%2Fjson&iothub-message-schema=application%2Fjson&iothub-content-encoding=utf-8&%24%24ContentEncoding=utf-8/", StringComparison.Ordinal) &&
                    x.QualityOfServiceLevel == MqttQualityOfServiceLevel.AtLeastOnce &&
                    string.Equals(Encoding.UTF8.GetString(x.Payload), payload, StringComparison.Ordinal) &&
                    x.Retain &&
        ...
        public async Task SendBrokerEventTest1()
        {
            ...
            mock.Verify(x => x.EnqueueAsync(
                It.Is<MqttApplicationMessage>(x =>
                    string.Equals(x.ContentType, "application/json", StringComparison.Ordinal) &&
                    string.Equals(x.Topic, "/topic/device1/testoutput", StringComparison.Ordinal) &&
                    x.QualityOfServiceLevel == MqttQualityOfServiceLevel.AtLeastOnce &&
                    string.Equals(Encoding.UTF8.GetString(x.Payload), payload, StringComparison.Ordinal) &&
                    x.Retain &&
        ...
        public async Task SendBrokerEventTest2()
        {
            ...
            mock.Verify(x => x.EnqueueAsync(
                It.Is<MqttApplicationMessage>(x =>
                    string.Equals(x.ContentType, "application/json", StringComparison.Ordinal) &&
                    string.Equals(x.Topic, "/topic/super", StringComparison.Ordinal) &&
                    x.QualityOfServiceLevel == MqttQualityOfServiceLevel.AtLeastOnce &&
                    string.Equals(Encoding.UTF8.GetString(x.Payload), payload, StringComparison.Ordinal) &&
                    !x.Retain &&
        ...
        public async Task GetTwinTest()
        {
            ...
            mock.SetupGet(x => x.IsStarted).Returns(true);
            var mqttClientAdapter = await MqttClientAdapter.CreateAsync(mock.Object, _mqttClientConnectionStringBuilder,
                "device1", "/topic/{device_id}", TimeSpan.FromSeconds(0), _logger, _metrics).ConfigureAwait(false);
            await mqttClientAdapter.GetTwinAsync().ConfigureAwait(false);
        ...
        public async Task ConnectCanceledTest()
        {
            ...
            mock.Verify(x => x.StartAsync(
                It.Is<ManagedMqttClientOptions>(x =>
                    x.ClientOptions.ChannelOptions is MqttClientTcpOptions &&
                    x.ClientOptions.ProtocolVersion == MqttProtocolVersion.V500 &&
                    string.Equals(x.ClientOptions.ChannelOptions.As<MqttClientTcpOptions>().Server, "hub1.azure-devices.net", StringComparison.Ordinal) &&
                    x.ClientOptions.ChannelOptions.As<MqttClientTcpOptions>().Port == 8883 &&
                    x.ClientOptions.ChannelOptions.As<MqttClientTcpOptions>().BufferSize == GetExpectedBufferSize() &&
                    x.ClientOptions.ChannelOptions.As<MqttClientTcpOptions>().TlsOptions != null &&
                    x.ClientOptions.ChannelOptions.As<MqttClientTcpOptions>().TlsOptions.UseTls &&
