...
namespace Microsoft.Azure.IIoT.OpcUa.Twin.Tests.Deploy {
    using Autofac;
    ...
    /// <summary>
    /// Test to check layered deployments that are generated by IoTHubSupervisorDeployment
    /// </summary>
    public class IoTHubSupervisorDeploymentTests {

        [Fact]
        public async Task TwinDeploymentRoutesTestAsync() {
            ...
            using (var mock = Setup(ioTHubConfigurationServicesMock)) {
                var twinDeploymentService = mock.Create<IoTHubSupervisorDeployment>();
                await twinDeploymentService.StartAsync();
...

                {
                    ...
                    Assert.Equal("__default-opctwin", configurationModel.Id);
                    Assert.Equal(2, configurationModel.Content.ModulesContent.Count);
                    Assert.Equal("FROM /messages/modules/twin/* INTO $upstream",
                        configurationModel.Content.ModulesContent["$edgeHub"]["properties.desired.routes.twinToUpstream"]);
                }
                {
                    // Check routes of layered deployment for Windows
                    var configurationModel = configurationModelList[1];
                    Assert.Equal("__default-opctwin-windows", configurationModel.Id);
                    Assert.Equal(2, configurationModel.Content.ModulesContent.Count);
                    Assert.Equal("FROM /messages/modules/twin/* INTO $upstream",
                        configurationModel.Content.ModulesContent["$edgeHub"]["properties.desired.routes.twinToUpstream"]);
                }
        ...
        public async Task ContainerRegistryConfigTestAsync() {
            ...
            using (var mock = Setup(ioTHubConfigurationServicesMock, configuration)) {
...

                {
                    ...
                    var module = (Newtonsoft.Json.Linq.JObject)configurationModel.Content.ModulesContent["$edgeAgent"]["properties.desired.modules.twin"];
                    Assert.Equal("custom.azurecr.io/customNamespace/iotedge/opc-twin:4.5.6.7",
