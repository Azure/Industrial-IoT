...
namespace Microsoft.Azure.IIoT.OpcUa.Edge.Publisher.Engine {
    ...
    /// </summary>
    internal class MonitoredItemMessage : IEncodeable {
        ...
        private void DecodeBinary(IDecoder decoder) {
            ...
            if ((MessageContentMask & (uint)MonitoredItemMessageContentMask.ExtensionFields) != 0) {
                var dictionary = (KeyValuePairCollection)decoder.ReadEncodeableArray("ExtensionFields", typeof(Opc.Ua.KeyValuePair));
            ...
            if ((MessageContentMask & (uint)MonitoredItemMessageContentMask.ExtensionFields) != 0) {
                if (ExtensionFields != null) {
                    ...
                    foreach (var item in ExtensionFields) {
                        dictionary.Add(new Opc.Ua.KeyValuePair(){
                            Key = item.Key,
                            Value = item.Value});
                    }
                    encoder.WriteEncodeableArray("ExtensionFields", dictionary.ToArray(), typeof(Opc.Ua.KeyValuePair));
                }
            }
        }
