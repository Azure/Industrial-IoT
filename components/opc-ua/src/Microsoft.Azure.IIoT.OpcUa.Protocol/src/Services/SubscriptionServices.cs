        ...
        internal sealed class SubscriptionWrapper : ISubscription {
            ...
            public async Task EnableAsync(Session session) {
                try {
                    ...
                    _logger.Verbose(e, "Failed to enable subscription");
                    Enabled = false;
                    Active = false;
                }
            }
            ...
            public async Task ActivateAsync(Session session) {
                try {
                    ...
                    _logger.Verbose(e, "Failed to activate subscription");
                    Enabled = false;
                    Active = false;
                }
            }
            ...
            private void ResolveDisplayNameAsync(IEnumerable<MonitoredItemModel> monitoredItems) {
                ...
                try {
                    ...
                    _logger.Warning("Failed to resolve display names for monitored items due to {StatusCode}", sre.StatusCode);
                }
                catch (Exception e) {
