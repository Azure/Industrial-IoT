    ...
    public sealed class OpcUaApplicationRegistration : OpcUaTwinRegistration {
        ...
        public DeviceTwinModel Patch(ApplicationInfoModel application, bool? disable = null) {
            ...
            var capabilities = application.Capabilities?
              .Select(x => x.SanitizePropertyName().ToUpperInvariant());
            if (!Capabilities.DecodeAsSet().SetEqualsSafe(capabilities)) {
                Capabilities = application.Capabilities.EncodeAsDictionary(true);
                twin.Tags.Add(nameof(Capabilities), JToken.FromObject(Capabilities));
            }
