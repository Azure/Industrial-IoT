...
namespace Microsoft.Azure.IIoT.Deployment.AKS.Deployment {
    ...
    using Microsoft.Azure.IIoT.Deployment.AKS.Resources;
    ...
    class DeploymentExecutor : IDisposable {
        ...
        protected void InitializeDefaultTags(string owner = null) {
            ...
            var valueIotSuiteType =
                $"{IIoTDeploymentTags.VALUE_APPLICATION_IIOT}-" +
                $"{versionIIoT}-" +
                $"{IIoTDeploymentTags.VALUE_IOT_SUITE_TYPE_SUF}";

            _defaultTagsList = new List<string> {
                IIoTDeploymentTags.VALUE_APPLICATION_IIOT,
                versionIIoT,
                IIoTDeploymentTags.VALUE_MANAGED_BY_IIOT,
                valueIotSuiteType
            };

            _defaultTagsDict = new Dictionary<string, string> {
                { IIoTDeploymentTags.KEY_APPLICATION, IIoTDeploymentTags.VALUE_APPLICATION_IIOT },
                { IIoTDeploymentTags.KEY_VERSION, versionIIoT},
                { IIoTDeploymentTags.KEY_MANAGED_BY, IIoTDeploymentTags.VALUE_MANAGED_BY_IIOT},
                { IIoTDeploymentTags.KEY_IOT_SUITE_TYPE, valueIotSuiteType}
            };

            if (null != owner) {
                _defaultTagsList.Add(owner);
                _defaultTagsDict.Add(IIoTDeploymentTags.KEY_OWNER, owner);
            }
        }
        ...
        protected async Task CreateAzureResourcesAsync(
            ...
            var omsAgentConfBlobUri = await UploadBlobAsync(
                ...
                IIoTK8SResources._04_oms_agent_configmap,
                cancellationToken
            );
            ...
            var clusterIssuerBlobUri = await UploadBlobAsync(
                ...
                IIoTK8SResources._90_letsencrypt_cluster_issuer,
                cancellationToken
            );
