...
namespace Microsoft.Azure.IoTSolutions.Browser.Controllers {
    ...
    public class BrowseController : Controller {
        ...
        public BrowseController(IOpcUaExplorerService explorer, ILogger logger) {
            _explorer = explorer;
            _logger = logger;
        }
        ...
        public async Task<ActionResult> GetRootNodeAsync() {
            ...
            var result = await _explorer.NodeBrowseAsync(endpointId, new BrowseRequestApiModel());
            return Json(new List<object> {
                new {
                    id = result.Node.Id,
                    text = Strings.BrowserRootNodeName,
        ...
        public async Task<ActionResult> VariableRead(string jstreeNode) {
            ...
            var result = await _explorer.NodeValueReadAsync(endpointId,
            ...
            var actionResult = 
                Strings.BrowserOpcDataValueLabel + ": " + value + @"<br/>" +
                Strings.BrowserOpcDataSourceTimestampLabel + ": " + result.SourceTimestamp + @"<br/>" +
                Strings.BrowserOpcDataServerTimestampLabel + ": " + result.ServerTimestamp;
        ...
        public async Task<ActionResult> VariablePublishUnpublish(string jstreeNode, string method) {
            ...
            var result = await _explorer.NodePublishAsync(endpointId,
                new PublishRequestApiModel {
                    NodeId = node,
                    Enabled = method != "unpublish"
                });
            var actionResult = Strings.BrowserOpcMethodCallSucceeded;
            if (result.Diagnostics != null) {
                actionResult = 
                    Strings.BrowserOpcMethodCallFailed + @"<br/><br/>" +
                    Strings.BrowserOpcDataDiagnosticInfoLabel + ": " + result.Diagnostics;
            }
            return Content(actionResult);
        }
        ...
        public async Task<ActionResult> VariableWriteUpdateAsync(
            ...
            var browseresult = await _explorer.NodeBrowseAsync(endpointId,
            ...
            if (browseresult.Node != null) {
                ...
                var result = await _explorer.NodeValueWriteAsync(endpointId,
            ...
            return Content(
                Strings.BrowserOpcMethodCallFailed + @"<br/><br/>" +
                Strings.BrowserOpcDataDiagnosticInfoLabel + ": " + diagnostics);
        }
        ...
        public async Task<ActionResult> MethodCallGetParameterAsync(
            ...
            var result = await _explorer.NodeMethodGetMetadataAsync(endpointId,
        ...
        public async Task<ActionResult> MethodCallAsync(string jstreeNode, 
            ...
            var result = await _explorer.NodeMethodCallAsync(endpointId,
            ...
            if ((result.Results?.Count ?? 0) == 0) {
                return Content(Strings.BrowserOpcMethodCallSucceeded);
            }
            var actionResult = Strings.BrowserOpcMethodCallSucceededWithResults + @"<br/><br/>";
        ...
        public async Task<ActionResult> Download() {
            ...
            var certString = await _explorer.GetClientCertificateAsync(endpointId);
