    ...
    public class DiscoveryEventProcessor : IEventProcessor {
        ...
        public async Task ProcessEventsAsync(PartitionContext context,
            ...
            foreach (var eventData in messages) {
                ...
                ServerEndpointDiscoveryModel discovery;
                try {
                    discovery = JsonConvertEx.DeserializeObject<ServerEndpointDiscoveryModel>(json);
                }
                catch (Exception ex) {
        ...
        private async Task ProcessServerEndpointDiscoveryAsync(
            ...
            ServerEndpointDiscoveryModel model, EventData checkpoint) {
            try {
                ...
                if (queue.Completed) {
                    ...
                    await _registry.ProcessSupervisorDiscoveryAsync(supervisorId,
                        queue.Endpoints.Select(x => x.ServerEndpoint));
        ...
        private class SupervisorDiscoveryResult {
            ...
            /// </summary>
            /// <returns></returns>
            public IEnumerable<ServerEndpointDiscoveryModel> Endpoints {
                get {
            ...
            public SupervisorDiscoveryResult(EventData checkpoint) {
                ...
                _endpoints = new List<ServerEndpointDiscoveryModel>();
                _maxIndex = 0;
            }
            ...
            /// <param name="model"></param>
            /// <returns></returns>
            public void Enqueue(ServerEndpointDiscoveryModel model) {
                _maxIndex = Math.Max(model.Index, _maxIndex);
                if (model.ServerEndpoint != null) {
            ...
            private readonly List<ServerEndpointDiscoveryModel> _endpoints;
