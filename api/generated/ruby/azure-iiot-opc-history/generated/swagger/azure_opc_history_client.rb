...
module azure.iiot.opc.history
  ...
  class AzureOpcHistoryClient < MsRest::ServiceClient
    ...
    def query_applications_by_id_async(query:nil, custom_headers:nil)
      ...
      # Serialize Request
      request_mapper = azure.iiot.opc.history::Models::ApplicationRecordQueryApiModel.mapper()
      request_content = self.serialize(request_mapper,  query)
      request_content = request_content != nil ? JSON.generate(request_content, quirks_mode: true) : nil
      ...
      promise = promise.then do |result|
        ...
        if status_code == 200
          begin
            parsed_response = response_content.to_s.empty? ? nil : JSON.load(response_content)
            result_mapper = azure.iiot.opc.history::Models::ApplicationRecordListApiModel.mapper()
            result.body = self.deserialize(result_mapper, parsed_response)
          rescue Exception => e
            fail MsRest::DeserializationError.new('Error occurred in deserializing the response', e.message, e.backtrace, result)
          end
