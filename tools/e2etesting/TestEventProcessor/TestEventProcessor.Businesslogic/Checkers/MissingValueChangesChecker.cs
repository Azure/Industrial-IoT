...
namespace TestEventProcessor.BusinessLogic.Checkers
{
    ...
    sealed class MissingValueChangesChecker : IDisposable
    {
        ...
        public void ProcessEvent(DateTime sourceTimestamp)
        {
            ...
            try
            {
                if (!_valueChangesPerTimestamp.TryGetValue(sourceTimestamp, out var value))
                {
                    value = 0;
                    _valueChangesPerTimestamp.Add(sourceTimestamp, value);
                }

                _valueChangesPerTimestamp[sourceTimestamp] = ++value;
            }
            finally
            {
                _lock.Release();
