...
namespace Microsoft.Azure.IIoT.Services.Common.Jobs.v2.Models {
    ...
    public class JobInfoApiModel {
        ...
        /// </summary>
        /// <returns></returns>
        internal JobInfoModel ToServiceModel() {
            return new JobInfoModel {
                Id = Id,
                Name = Name,
                JobConfiguration = JobConfiguration?.DeepClone(),
                JobConfigurationType = JobConfigurationType,
                Demands = Demands?.Select(d => d.ToServiceModel()).ToList(),
                LifetimeData = LifetimeData?.ToServiceModel(),
                RedundancyConfig = RedundancyConfig?.ToServiceModel()
            };
        }
