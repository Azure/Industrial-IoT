    ...
    public class OpcUaRegistryMaintenanceTests {
        ...
        private static List<ApplicationRegistrationModel> ApplicationsIn(IoTHubDeviceRegistry registry) {
            var registrations = registry.Devices
                .Select(d => d.Twin)
                .Select(OpcUaTwinRegistration.ToRegistration)
                .ToList();
            var endpoints = registrations
                .OfType<OpcUaEndpointRegistration>()
                .GroupBy(d => d.ApplicationId)
                .ToDictionary(
                    k => k.Key,
                    v => v.Select(e => e.ToServiceModel().Registration).ToList());
            return registrations
                .OfType<OpcUaApplicationRegistration>()
        ...
        private static void CreateFixtures(out string site, out string super,
            ...
            var appdevices = existing
                .Select(a => OpcUaApplicationRegistration.FromServiceModel(a.Application, disable))
                .Select(a => OpcUaApplicationRegistration.Patch(null, a))
            ...
            var epdevices = existing
                .SelectMany(a => a.Endpoints
                    .Select(e => OpcUaEndpointRegistration.FromServiceModel(
                        new TwinInfoModel {
                            ApplicationId = a.Application.ApplicationId,
                            Registration = e
                        }, disable)))
                .Select(e => OpcUaEndpointRegistration.Patch(null, e))
                .Select(d => new IoTHubDeviceModel {
                    Device = new DeviceModel {
                        Id = d.Id
                    },
