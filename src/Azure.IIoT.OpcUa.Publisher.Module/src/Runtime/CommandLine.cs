        ...
        public CommandLine(string[] args, CommandLineLogger? logger = null)
        {
            ...
            var options = new CommandLineOptionParser
            {
                ...
                { $"rdb|removedupsinbatch:|{PublisherConfig.RemoveDuplicatesFromBatchKey}:",
                    "Use this option to remove values with the same node id from batch messages. This applied only to the `Samples` mode that was removed in 3.0, so the option no longer has an effect.\nDeprecated, it is scheduled for removal.\nDefault: `false` (keep all duplicate values).\n",
                ...
                { $"unp|usenativepubsub:|{PublisherConfig.UseNativePubSubKey}:",
                    "EXPERIMENTAL/PREVIEW: Use the native OPC UA PubSub runtime for telemetry publishing instead of the current custom encoder sink.\nThe transport is selected per writer group exactly as the default publishing path selects it. A transport that cannot carry a message schema publishes without one and logs a warning; a delivery guarantee that cannot be honoured is still rejected.\nDefault: `false` (disabled; existing telemetry path is unchanged).\n",
