    ...
    public sealed class CommandLine : Dictionary<string, string?>
    {
        ...
        public CommandLine(string[] args, CommandLineLogger? logger = null)
        {
            ...
            var options = new Mono.Options.OptionSet
            {
                ...
                { $"cdi|clientdiagnosticinterval=|{OpcUaClientConfig.ClientDiagnosticsIntervalKey}=",
                    "When enabled in the configuration diagnostics for the connection are dumped to the console every n seconds.\nDefault: `30` seconds when enabled.\n",
                    (uint i) => this[OpcUaClientConfig.ClientDiagnosticsIntervalKey] = TimeSpan.FromSeconds(i).ToString() },
