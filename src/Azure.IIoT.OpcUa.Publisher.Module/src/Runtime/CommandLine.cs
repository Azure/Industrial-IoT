        ...
        public CommandLine(string[] args, CommandLineLogger? logger = null)
        {
            ...
            var options = new Mono.Options.OptionSet
            {
                ...
                { $"p|httpserverport=|{PublisherConfig.HttpServerPortKey}=",
                    $"The port on which the http server of OPC Publisher is listening.\nDefault: `{PublisherConfig.HttpsPortDefault}` if no value is provided.\n",
                    p => this[PublisherConfig.HttpServerPortKey] = p },
                { $"unsecurehttp:|{PublisherConfig.UnsecureHttpServerPortKey}:",
                    $"Allow unsecure access to the REST api of OPC Publisher. A port can be specified if the default port {PublisherConfig.HttpPortDefault} is not desired.\nDo not enable this in production as it exposes the Api Key on the network.\nDefault: `disabled`, if specified without a port `{PublisherConfig.HttpPortDefault}` port is used.\n",
                    (ushort? p) => this[PublisherConfig.UnsecureHttpServerPortKey] = p?.ToString(CultureInfo.CurrentCulture) ?? PublisherConfig.HttpPortDefault.ToString(CultureInfo.CurrentCulture) },
