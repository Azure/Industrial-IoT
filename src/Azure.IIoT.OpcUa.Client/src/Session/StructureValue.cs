...
namespace Opc.Ua.Client.Dynamic;
    ...
    IStructureTypeInfo, IDynamicComplexTypeInstance
{
    ...
    public StructureValue(ExpandedNodeId? typeId = null)
    {
        TypeId = typeId ?? ExpandedNodeId.Null;
    }
    ...
    private StructureValue(StructureValue encodeable)
    {
    ...
    {
        if (encoder.Context?.Factory is DataTypeSystem f)
        {
            _typeCache = f;
        }
    ...
    {
        if (decoder.Context?.Factory is DataTypeSystem f)
        {
            _typeCache = f;
        }
        var typeInfo = GetTypeInfo();
    ...
    {
        if (context.Factory is DataTypeSystem f)
        {
            _typeCache = f;
        }
    ...
    private DataTypeSystem? _typeCache;
}
