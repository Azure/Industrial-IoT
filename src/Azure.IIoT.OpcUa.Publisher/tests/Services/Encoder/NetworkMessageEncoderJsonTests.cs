...
namespace Azure.IIoT.OpcUa.Publisher.Tests.Services
{
    ...
    public class NetworkMessageEncoderJsonTests
    {
        ...
        public void EncodeChunkTest(bool encodeBatchFlag)
        {
            ...
            // The restored 2.x payload shape gets back to 94 processed
            // notifications (the fork handled 95); the System.Text.Json envelope
            // produces a small batch/no-batch chunk-count delta.
            Assert.InRange(count, 64, 68);
            Assert.Equal(94, encoder.NotificationsProcessedCount);
            Assert.Equal((uint)500 - 94, encoder.NotificationsDroppedCount);
