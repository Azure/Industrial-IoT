...
namespace Microsoft.Azure.IIoT.OpcUa.Edge.Control {
    ...
    internal static class Generator {
        ...
        public static void WriteResponse(BrowseResultModel results) {
            ...
            Assert.Equal(""{results.Node.Id}"", results.Node.Id);
            ...
            foreach (var reference in results.References) {
                references +=
$@"                reference => {{
                    Assert.Equal(""{reference.Id}"", reference.Id);
                    Assert.Equal(""{reference.BrowseName}"", reference.BrowseName);
                    Assert.Equal(""{reference.DisplayName}"", reference.DisplayName);
                    ...
                    Assert.Equal(""{reference.Target.Id}"",
                        reference.Target.Id);
        ...
        internal static void WriteResponse(MethodMetadataResultModel results, [CallerMemberName] string methodname = null) {
            ...
            foreach (var arg in results.InputArguments) {
                ...
                arg => {{
                    ...
                    Assert.Equal(""{arg.Type.Id}"", arg.Type.Id);
