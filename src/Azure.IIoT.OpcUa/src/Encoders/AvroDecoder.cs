...
namespace Azure.IIoT.OpcUa.Encoders
{
    ...
    public sealed class AvroDecoder : IDecoder
    {
        ...
        /// <summary>
        /// Read integer in ziz zag encoding
        /// </summary>
        /// <returns></returns>
        ...
        internal void ReadFixed(Span<byte> buffer)
        {
        ...
        {
            return ReadNullable(ReadString);
        }
        ...
        internal string ReadString()
        {
            ...
            var maxStringLength = Context.MaxStringLength;
        ...
        {
            return ReadNullable(ReadByteString);
        }

        /// <summary>
        /// Read non nullable byte string
        /// </summary>
        /// <returns></returns>
        /// <exception cref="ServiceResultException"></exception>
        internal byte[] ReadByteString()
        {
            ...
            var maxByteStringLength = Context.MaxByteStringLength;
        ...
        public NodeId ReadNodeId(string? fieldName)
        {
            ...
            var namespaceIndex = (ushort)ReadInteger();
            ...
            switch ((IdType)ReadInteger()) // Union field id
            {
                case IdType.Numeric:
                    return new NodeId(ReadUInt32(null), namespaceIndex);
                case IdType.String:
                    return new NodeId(ReadString(), namespaceIndex);
                case IdType.Guid:
                    return new NodeId(ReadGuid(null), namespaceIndex);
                case IdType.Opaque:
                    return new NodeId(ReadByteString(), namespaceIndex);
                default:
                    throw ServiceResultException.Create(StatusCodes.BadDecodingError,
        ...
        {
            return new LocalizedText(ReadString(), ReadString());
        }
        ...
        {
            return (Enum)Enum.ToObject(enumType, ReadInteger());
        }
        ...
        private ExtensionObject ReadExtensionObject()
        {
            ...
            switch (unionId)
            {
                ...
                case 2:
                    // 2.3
                    switch ((ExtensionObjectEncoding)ReadInteger())
                    {
                        case ExtensionObjectEncoding.Binary:
                            return new ExtensionObject(expandedTypeId, ReadByteString());
                        ...
                        case ExtensionObjectEncoding.Json:
                            return new ExtensionObject(expandedTypeId, ReadString());
                        default:
                            throw ServiceResultException.Create(StatusCodes.BadDecodingError,
        ...
        private Variant ReadVariantValue()
        {
            ...
            if (isArray)
            {
                ...
                else
                {
                    ...
                    if (dimensionsArray.Length > 1) // Value Rank > 1
                    {
                        (var valid, var matrixLength) = Matrix.ValidateDimensions(
                            dimensionsArray, length, Context.MaxArrayLength);
                        ...
                        value = new Variant(new Matrix(array,
                            builtInType, dimensions.ToArray()));
                    }
                    else
                    {
