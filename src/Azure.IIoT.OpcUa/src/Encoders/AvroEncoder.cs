...
namespace Azure.IIoT.OpcUa.Encoders
{
    ...
    /// <summary>
    /// Encodes objects in a stream using Avro encoding.
    /// </summary>
    public sealed class AvroEncoder : IEncoder
    {
        ...
        /// <summary>
        /// Writes fixed-length binary data
        /// </summary>
        /// <param name="value"></param>
        internal void WriteFixed(ReadOnlySpan<byte> value)
        {
            _stream.Write(value);
        }
        ...
        {
            WriteFixed(((Guid)value).ToByteArray());
        }

        /// <inheritdoc/>
        public void WriteGuid(string? fieldName, Guid value)
        {
            WriteFixed(value.ToByteArray());
        }
        ...
        internal void WriteString(string value)
        {
            ...
            if (Context.MaxStringLength > 0
                && Context.MaxStringLength < bytes.Length)
            {
        ...
        public void WriteNodeId(string? fieldName, NodeId? value)
        {
            ...
            //  Guid = 2
            //  Opaque = 3
            WriteInteger((int)idUnionIndex);
            switch (idUnionIndex)
            {
                case IdType.Numeric:
                    WriteInteger((uint)(value?.Identifier ?? 0u));
                    break;
                case IdType.String:
                    WriteString((string)value!.Identifier);
                    break;
                case IdType.Guid:
                    WriteGuid(null, (Guid)value!.Identifier);
                    break;
                case IdType.Opaque:
                    WriteByteString((byte[])value!.Identifier);
                    break;
            }
        }
            ...
            ExpandedNodeId? value)
        {
            ...
            WriteString(value?.NamespaceUri ?? string.Empty);
            WriteInteger(serverIndex);
        }
        ...
        internal void WriteQualifiedName(QualifiedName value)
        {
            ...
            WriteInteger(namespaceIndex);
            WriteString(value.Name ?? string.Empty);
        }
        ...
        {
            WriteString(null, value.Locale); // Nullable
            WriteString(value.Text ?? string.Empty); // Not nullable
        }
        ...
        internal void WriteDataValue(DataValue value)
        {
            ...
            WriteInteger(value.SourcePicoseconds);
            WriteDateTime(null, value.ServerTimestamp);
            WriteInteger(value.ServerPicoseconds);
        }
    ...
    "Cannot encode bodies of type '{0}' in ExtensionObject unless the NamespaceUri ({1}) is in the encoder's NamespaceTable.",
            ...
            WriteInteger(2);
            // 2.3.1
            WriteInteger((int)value.Encoding);
            // 2.3.2
            switch (body)
            {
                case byte[] buffer:
                    // write binary bodies.
                    WriteByteString(buffer);
                    break;
                case XmlElement xml:
                    // write XML bodies.
                    WriteXmlElement(xml);
        ...
        {
            WriteInteger(Convert.ToInt32(value, CultureInfo.InvariantCulture));
        }
        ...
        private void WriteDiagnosticInfo(DiagnosticInfo value, int depth = 0)
        {
            ...
            try
            {
                WriteInteger(value.SymbolicId);
                WriteInteger(value.NamespaceUri);
                WriteInteger(value.Locale);
                WriteInteger(value.LocalizedText);
                WriteString(value.AdditionalInfo ?? string.Empty);
        ...
        /// <exception cref="ServiceResultException"></exception>
        private bool WriteArrayLength([NotNullWhen(false)] Array? values)
        {
            // check for null.
            if (values == null)
            {
            ...
            if (Context.MaxArrayLength > 0 &&
                Context.MaxArrayLength < values.Length)
            {
                throw ServiceResultException.Create(
                    StatusCodes.BadEncodingLimitsExceeded,
                    "MaxArrayLength {0} < {1}",
        ...
        private void WriteVariantValue(Variant value)
        {
            ...
            if (value.TypeInfo!.ValueRank < 0)
            {
                ...
                switch (value.TypeInfo.BuiltInType)
                {
                    ...
                    case BuiltInType.Double:
                        ...
                        return;
                    case BuiltInType.String:
                        WriteString((string)valueToEncode);
                        return;
                    case BuiltInType.DateTime:
                    ...
                    case BuiltInType.Guid:
                        ...
                        return;
                    case BuiltInType.ByteString:
                        WriteByteString((byte[])valueToEncode);
                        return;
                    case BuiltInType.XmlElement:
            ...
            if (value.TypeInfo.ValueRank >= 0)
            {
                ...
                WriteInteger(ToUnionId(value.TypeInfo.BuiltInType, true));
                switch (value.TypeInfo.BuiltInType)
                {
                    case BuiltInType.Boolean:
