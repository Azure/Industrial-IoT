...
namespace Azure.IIoT.OpcUa.Encoders
{
    ...
    public class VariantEncoderByteTests
    {
        ...
        {
            var codec = new JsonVariantEncoder(new ServiceMessageContext());
            var str = TestJson.FromArray((byte)123, (byte)124, (byte)125);
            var variant = codec.Decode(str, BuiltInType.Byte);
        ...
        {
            var codec = new JsonVariantEncoder(new ServiceMessageContext());
            var str = TestJson.FromObject("{|}"u8.ToArray());
        ...
        public void DecodeEncodeByteArrayFromVariantJsonTokenTypeVariant3()
        {
            ...
            var encoded = codec.Encode(variant);
            Assert.Equal(expected, variant);
            Assert.True(JsonNode.DeepEquals(TestJson.FromObject("{|}"u8.ToArray()), encoded));
        }
