...
// and copies, so it's a #define opt-in.
                ...
                var blocks = checked(_offset + pendingCount + (BlockSize - 1)) / BlockSize;
                var newBytes = ArrayPool<byte>.Shared.Rent(BlockSize * blocks);
                ...
                for (var i = _offset; i < newBytes.Length; i++) {
                    newBytes[i] ^= 0xFF;
                }
#endif
        ...
        private void WriteTag(Asn1Tag tag) {
            ...
            if (!tag.TryWrite(_buffer.AsSpan(_offset, spaceRequired), out var written) ||
        ...
        public void WriteEncodedValue(ReadOnlyMemory<byte> preEncodedValue) {
            var reader = new AsnReader(preEncodedValue, RuleSet);

            // Is it legal under the current rules?
            var parsedBack = reader.GetEncodedValue();
        ...
        private void WriteConstructedCerBitString(Asn1Tag tag, ReadOnlySpan<byte> payload, int unusedBitCount) {
            ...
            var fullSegments = Math.DivRem(payload.Length, MaxCERContentSize, out var lastContentSize);
            ...
            var expectedSize = (fullSegments * FullSegmentEncodedSize) + remainingEncodedSize + 2;
            EnsureWriteCapacity(expectedSize);

            var ensureNoExtraCopy = _buffer;
            var savedOffset = _offset;

            var remainingData = payload;
            Span<byte> dest;
            var primitiveBitString = Asn1Tag.PrimitiveBitString;
        ...
        private void WriteNamedBitList(Asn1Tag tag, ulong integralValue) {
            ...
            var indexOfHighestSetBit = -1;

            for (var i = 0; integralValue != 0; integralValue >>= 1, i++) {
            ...
            if (indexOfHighestSetBit < 0) {
                ...
                var byteLen = (indexOfHighestSetBit / 8) + 1;
                var unusedBitCount = 7 - (indexOfHighestSetBit % 8);
        ...
        private void WriteConstructedCerOctetString(Asn1Tag tag, ReadOnlySpan<byte> payload) {
            ...
            var fullSegments = Math.DivRem(payload.Length, MaxCERSegmentSize, out var lastSegmentSize);
            ...
            var remainingData = payload;
            Span<byte> dest;
            var primitiveOctetString = Asn1Tag.PrimitiveOctetString;
        ...
        private void WriteObjectIdentifierCore(Asn1Tag tag, ReadOnlySpan<char> oidValue) {
            ...
            var tmp = ArrayPool<byte>.Shared.Rent(oidValue.Length / 2);
            var tmpOffset = 0;
            ...
            try {
                ...
                var localLen = EncodeSubIdentifier(tmp.AsSpan(tmpOffset), ref subIdentifier);
                tmpOffset += localLen;
        ...
        private static BigInteger ParseSubIdentifier(ref ReadOnlySpan<char> oidValue) {
            ...
            for (var position = 0; position < endIndex; position++) {
        ...
        private static int EncodeSubIdentifier(Span<byte> dest, ref BigInteger subIdentifier) {
            ...
            var unencoded = subIdentifier;
            var idx = 0;

            do {
                var cur = unencoded & 0x7F;
        ...
        private void PopSetOfCore(Asn1Tag tag) {
            ...
            var sortContents = RuleSet == AsnEncodingRules.CER || RuleSet == AsnEncodingRules.DER;
        ...
        private void WriteUtcTimeCore(Asn1Tag tag, DateTimeOffset value) {
            ...
            var month = normalized.Month;
            var day = normalized.Day;
            var hour = normalized.Hour;
            var minute = normalized.Minute;
            var second = normalized.Second;

            var baseSpan = _buffer.AsSpan(_offset);
            var format = new StandardFormat('D', 2);
        ...
        private void WriteGeneralizedTimeCore(Asn1Tag tag, DateTimeOffset value, bool omitFractionalSeconds) {
            ...
            if (!omitFractionalSeconds) {
                var floatingTicks = normalized.Ticks % TimeSpan.TicksPerSecond;
                ...
                if (floatingTicks != 0) {
                    ...
                    if (!Utf8Formatter.TryFormat(decimalTicks, fraction, out var bytesWritten, new StandardFormat('G'))) {
            ...
            var totalLength = IntegerPortionLength + 1 + fraction.Length;
            ...
            var baseSpan = _buffer.AsSpan(_offset);
            var d4 = new StandardFormat('D', 4);
            var d2 = new StandardFormat('D', 2);
        ...
        private void PopTag(Asn1Tag tag, bool sortContents = false) {
            ...
            (var stackTag, var lenOffset) = _nestingStack.Peek();
            ...
            var containedLength = _offset - 1 - lenOffset;
        ...
        private void WriteCharacterStringCore(Asn1Tag tag, Text.Encoding encoding, ReadOnlySpan<char> str) {
            ...
            unsafe {
                fixed (char* strPtr = &MemoryMarshal.GetReference(str)) {
                    ...
                    WriteTag(tag.AsPrimitive());
                    WriteLength(size);
                    var dest = _buffer.AsSpan(_offset, size);

                    fixed (byte* destPtr = &MemoryMarshal.GetReference(dest)) {
                        var written = encoding.GetBytes(strPtr, str.Length, destPtr, dest.Length);
            ...
            unsafe {
                fixed (char* strPtr = &MemoryMarshal.GetReference(str)) {
                    ...
                    fixed (byte* destPtr = tmp) {
                        var written = encoding.GetBytes(strPtr, str.Length, destPtr, tmp.Length);
        ...
        private static void SortContents(byte[] buffer, int start, int end) {
            ...
            while (reader.HasData) {
                var encoded = reader.GetEncodedValue();
            ...
            var tmp = ArrayPool<byte>.Shared.Rent(len);

            pos = 0;

            foreach ((var offset, var length) in positions) {
        ...
        private class ArrayIndexSetOfValueComparer : IComparer<(int, int)> {
            ...
            public int Compare((int, int) x, (int, int) y) {
                (var xOffset, var xLength) = x;
                (var yOffset, var yLength) = y;
