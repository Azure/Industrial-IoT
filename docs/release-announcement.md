...
- Windows container images are no longer supported in IoT Edge 1.4 and consequentially have been removed from this release. Please use [IoT Edge for Linux on Windows (EFLOW) 1.4](https://learn.microsoft.com/en-us/windows/iot/iot-enterprise/azure-iot-edge-for-linux-on-windows) as your IoT Edge environment on Windows.  
  > IoT Edge 1.4 LTS EFLOW is supported as Preview Feature in this release (see Known Issues below).
...
- Under EFLOW, by default the Application instance certificate is generated by each module and not shared.
  > To work around this, configure EFLOW and modules deployed manually to use a shared host volume to map the pki folders.
- Discoverying using IP Address and port ranges not supported on EFLOW.
