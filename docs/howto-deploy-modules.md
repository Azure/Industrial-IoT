...
4. Validate the presence of `host` network in the docker instance 
	...
	When running the industiral iot edge modules in host (transparent) network, they will require IP addresses assignment. There are 2 options to do so 
	* from a local DHCP server accessible from the host's network interface assciated to the container's 'host' network.  
	* a valid static IP address assigned on the container create options statement
