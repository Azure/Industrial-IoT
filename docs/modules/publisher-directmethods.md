...
The following direct methods are inherited from 2.5.x in addition to some new ones:
...
To understand the terminologies and migration path from version 2.5.x of OPC Publisher to 2.8.2s, please refer to [this](publisher-dmmigration.md) migration document.
...
PublishNodes enables a client to add a set of nodes to be published for a specific DataSetWriter. When a DataSetWriter already exists, the nodes shall be incrementally added to the very same dataset. When it does not already exist, a new DataSetWriter shall be created with the initial set of nodes contained in the request.
  ...
  *Request:* follows strictly the request payload schema as described, the OpcNodes attribute being mandatory.
