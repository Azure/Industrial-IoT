...
## Build and run the Industrial IoT CLI Console

1. To run the CLI you first will clone the API repository.  Open a command prompt or terminal.  Run

   ```bash
   git clone https://github.com/Azure/azure-iiot-services-api 
   cd azure-iiot-services-api
   ```

   Copy the `.env` file that was generated during deployment into the current folder.
