...
> IMPORTANT: `WatchdogLKVWithUpdatedTimestamps` is the only behavior that *synthesizes* a source timestamp. The shift it applies is measured on the OPC Publisher wall clock, while the timestamp it is added to comes from the server clock. The constant offset between the two clocks cancels out, but network latency, publish cycle phase and processing delay do not - they are injected directly into the emitted `SourceTimestamp`. Because the shifted timestamp is not a duplicate of the previous one, a consumer that does not evaluate the `Heartbeat` indicator - which is only present in the full featured message profiles (`--fm=true`) - cannot distinguish such a heartbeat from a real sample. The shift is also measured from the time the value was *received*, which is already later than the timestamp the server assigned to it, so a heartbeat firing shortly before the next real value carries a timestamp *beyond* the one that value will carry and the observed timestamp sequence moves backwards. Prefer the default `WatchdogLKV` behavior, which re-sends the original timestamp unchanged, whenever the consumer analyzes `SourceTimestamp` spacing. See [Jittered timestamps without duplicates](./troubleshooting.md#jittered-timestamps-without-duplicates).
